...
class ViewUsers extends ViewRecord
{
    ...
    protected static string $view = 'Users::user-details';
    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\EditAction::make()
                ...
                ->action(fn(UsersCoreModel $record, $data) => (new UsersManagementService())->updateUser($record->id, $data))
                ->slideOver()
...
//        $basicInfo[] = TextEntry::make('code')->label(__("Bhry98::users.code"))->weight(FontWeight::Bold)->copyable();
        ...
        $accountInfo[] = TextEntry::make('email')->label(__("Bhry98::users.email"))->weight(FontWeight::Bold)->copyable()->icon(fn($record) => $record->email_verified_at ? 'heroicon-o-check-circle' : 'heroicon-o-exclamation-triangle')->iconColor(fn($record) => $record->email_verified_at ? 'success' : 'warning')->iconPosition(IconPosition::After)->extraAttributes(['class' => 'flex items-center gap-2']);
        $accountInfo[] = PhoneEntry::make('phone_number')->label(__("Bhry98::users.phone-number"))->weight(FontWeight::Bold)->copyable()->icon(fn($record) => $record->phone_number_verified_at ? 'heroicon-o-check-circle' : 'heroicon-o-exclamation-triangle')->iconColor(fn($record) => $record->phone_number_verified_at ? 'success' : 'warning')->iconPosition(IconPosition::After)->default("---");
        ...
        $accountInfo[] = TextEntry::make('phone_number_verified_at')->label(__("Bhry98::users.phone-number-verified-at"))->getStateUsing(fn($record) => $record->phone_number_verified_at ? Carbon::parse($record->phone_number_verified_at)->format(config("bhry98.date.format")) : "---")->weight(FontWeight::Bold)->copyable();
