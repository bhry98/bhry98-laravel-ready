...
class EnumsManagementService extends BaseService
{
     ...
     * @return EnumsCoreModel|null
     */
    public function getByCode(string $code, array|null $relations = null, bool $withRelationsCount = false, bool $withTrash = false): ?EnumsCoreModel
    {
        $record = EnumsCoreModel::query()->where(['code' => $code]);
     ...
     * @return EnumsCoreModel|null
     */
    public function getById(int $id, array|null $relations = null, bool $withRelationsCount = false, bool $withTrash = false): ?EnumsCoreModel
    {
        $record = EnumsCoreModel::query()->where(['id' => $id]);
        if ($withTrash) $record->withTrashed();
        if ($relations) $record->with($relations);
        if ($withRelationsCount) $record->withCount(['children']);
        return $record->first();
    ...
    {
        $data = EnumsCoreModel::query()->latest('id');
     ...
     * @return EnumsCoreModel|null
     */
    public function getDefault(string $enumTypeCode, string $enumDefaultName, ?string $enumModule = null, array|null $relations = null, bool $withTrash = false): ?EnumsCoreModel
    {
        $record = EnumsCoreModel::query()->where(["type" => $enumTypeCode, "default_name" => $enumDefaultName]);
        if ($relations) $record->with($relations);
        if ($withTrash) $record->withTrashed();
        return $record->first();
    }
    ...
    {
        $list = EnumsCoreModel::with('localizations');
