...
if (!function_exists(function: 'bhry98_common_database_columns')) {
    function bhry98_common_database_columns(Blueprint $table, bool $softDeletes = false, bool $userLog = false, bool $active = false, bool $note = false): void
    {
        ...
        if ($userLog) $table->foreignId(column: 'created_by')->nullable()->references(column: "id")->on(table: UsersCoreModel::TABLE_NAME)->cascadeOnUpdate()->cascadeOnDelete();
        if ($userLog) $table->foreignId(column: 'updated_by')->nullable()->references(column: "id")->on(table: UsersCoreModel::TABLE_NAME)->cascadeOnUpdate()->cascadeOnDelete();
        if ($userLog && $softDeletes) $table->foreignId(column: 'deleted_by')->nullable()->references(column: "id")->on(table: UsersCoreModel::TABLE_NAME)->cascadeOnUpdate()->cascadeOnDelete();
