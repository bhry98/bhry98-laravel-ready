    ...
    public function up(): void
    {
        ...
        Schema::create(
            ...
            callback: function (Blueprint $table) {
                $table->id();
                $table->uuid(column: 'identity_code')
                    ->unique();
                ...
                $table->foreignId(column: 'gender_id')
                    ...
                    ->cascadeOnUpdate()
                    ->cascadeOnDelete();
                $table->foreignId(column: 'timezone_id')
                    ->references(column: 'id')
                    ->on(table: EnumsCoreModel::TABLE_NAME)
                    ->cascadeOnUpdate()
                    ->cascadeOnDelete();
                $table->foreignId(column: 'country_id')
                    ->nullable()
                    ->references(column: 'id')
                    ->on(table: \Bhry98\Bhry98LaravelReady\Models\locations\LocationsCountriesModel::TABLE_NAME)
                    ->cascadeOnUpdate()
                    ->cascadeOnDelete();
                $table->foreignId(column: 'governorate_id')
                    ->nullable()
                    ->references(column: 'id')
                    ->on(table: \Bhry98\Bhry98LaravelReady\Models\locations\LocationsGovernoratesModel::TABLE_NAME)
                    ->cascadeOnUpdate()
                    ->cascadeOnDelete();
                $table->foreignId(column: 'city_id')
                    ->nullable()
                    ->references(column: 'id')
                    ->on(table: \Bhry98\Bhry98LaravelReady\Models\locations\LocationsCitiesModel::TABLE_NAME)
                    ->cascadeOnUpdate()
                    ->cascadeOnDelete();
